...
is restricted to the carrier-basis pairs used in the lower-bound construction.

Once a core carrier-basis pair is fixed,
the regime assignment is determined.
Choosing the identity basis for a real-world referent remains a matter of
modeling judgment.
...
\label{se200.def.CoreCarrierBasisPair}
...
\label{se200.def.CoreRegimeAssignment}
The \emph{core regime assignment} maps each core carrier-basis pair
to the identity regime displayed in Definition~\ref{se200.def.CoreCarrierBasisPair}.
\end{definition}
...
Every core carrier-basis pair is assigned to a unique core identity regime.
\end{theorem}
...
Therefore every core carrier-basis pair is assigned to a unique core regime.
\end{proof}
...
whether forking preserves locus or object identity.
