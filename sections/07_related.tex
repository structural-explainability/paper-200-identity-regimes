...
\subsection{Upper Ontologies} % REVISED TO CLARIFY RELATION TO BFO AND DOLCE
...
K4 (time-indexed occurrences) maps cleanly onto BFO occurrents.
K1 through K3 map onto independent and generically dependent continuants,
but BFO does not enforce the regime-level separation between them:
obligation-bearing entities (K1), authority-bearing structures (K3),
and descriptive indicators (K6) are all candidates for
Generically Dependent Continuant, leaving their
stability-critical distinctions to the implementer.
K5 (applicability and scope) has no first-class BFO home;
non-spatial applicability contexts such as normative jurisdiction
or conditional scope collapse into GDC,
conflating K5 with K3 under a BFO-based implementation.
...
This makes DOLCE a better fit for K2 and K3 as distinct categories.
However, the K3/K6 conflation persists: both normative structures and
...
leaving normative applicability contexts (K5) without a
structural home distinct from K3 or K6,
and the K2/K3 boundary (between obligation-bearer and
