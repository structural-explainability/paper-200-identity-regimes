...
\begin{definition}[Identity carrier]
  \label{se200.def.IdentityCarrier}
...
Therefore, the reference kind ``rule'' does not fix a single identity regime;
transformation behavior does.
...
A scope is the applicability domain of a rule, including
places, parties, periods, procedures, classifications, or conditions.
...
Therefore, the reference kind ``scope'' does not fix a single identity regime;
transformation behavior does.
...
Forking can separate those bases.
Therefore, the reference kind ``plain referent'' does not fix a single identity regime;
transformation behavior does.
